...
class BaseConverter(QWidget):
    def __init__(self):
        ...
        self.xor_button = QPushButton("XOR")  # Add XOR button
        self.shift_left_button = QPushButton("<<") # Add SHL button
        self.shift_right_button = QPushButton(">>") # Add SHR button
        ...
        op_layout.addWidget(self.xor_button)  # Add XOR button
        op_layout.addWidget(self.shift_left_button) # Add SHL button
        op_layout.addWidget(self.shift_right_button) # Add SHR button
    ...
    def perform_not(self):
        ...
        try:
            if self.current_input_mode == "hex":
                operand = int(current_text, 16)
                num_bits = 16  # 16-bit NOT for hexadecimal (more C-64 relevant)
            elif self.current_input_mode == "bin":
                operand = int(current_text, 2)
                num_bits = len(current_text) if current_text else 8 # Use length or default
            else: # decimal
                operand = int(current_text)
                if operand >= -(2**15) and operand <= (2**15 - 1):
                    num_bits = 16
                else:
                    num_bits = 8 # Default to 8 for smaller numbers
    ...
    def perform_equals(self):
        ...
        try:
            ...
            if self.current_operation == "+":
                ...
                result = self.first_operand << 1  # Assuming shift by 1 for now
