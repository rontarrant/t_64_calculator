...
class T64Calculator(QWidget):
	...
	def initialize_ui(self):
		...
		self.signed_button = QPushButton("Signed")
		...
		self.bit_size = 32
		self.entered_base = 10 # Tracks base of the numbers currently on display.
	...
	def create_button_click_handler(self, button_text):
		def handle_click():
			if button_text.isdigit() or button_text in "ABCDEF.()+-*/.":
				...
				self.convert_and_set_base(button_text)
	...
	def convert_and_set_base(self, base_text):
		try:
			current_text = self.display.text()
			if current_text:
				decimal_value = int(current_text, self.entered_base)
				if base_text == "Hex":
					self.display.setText(hex(decimal_value).upper().replace("0X", ""))
					self.current_base = 16
				elif base_text == "Dec":
					self.display.setText(str(decimal_value))
					self.current_base = 10
				elif base_text == "Oct":
					self.display.setText(oct(decimal_value).replace("0o", ""))
					self.current_base = 8
				elif base_text == "Bin":
					self.display.setText(bin(decimal_value).replace("0b", ""))
					self.current_base = 2
				self.entered_base = self.current_base
			else:
				self.set_base(base_text)

		except ValueError:
			self.display.setText("Error")
			self.current_base = 10
			self.entered_base = 10
			self.update_button_states()
		self.update_button_states()
	...
	def toggle_signed_unsigned(self):
		...
		if self.is_signed:
			self.signed_button.setText("Unsigned")
		else:
			self.signed_button.setText("Signed")
	...
	def calculate_result(self):
		try:
			...
			# This eval is unsafe for non trusted input.
			result = eval(expression)
			result = int(result)
			result = self.apply_bit_range(result)

			if self.entered_base == 16:
				result = hex(result).upper().replace("0X", "")
			elif self.entered_base == 8:
				result = oct(result).replace("0o", "")
			elif self.entered_base == 2:
