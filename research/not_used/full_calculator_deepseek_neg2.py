...
class CalculatorLogic:
    ...
    def get_display_value(self, num=None):
        try:
            ...
            negative = num < 0
            num = abs(self._apply_bit_width_and_sign(num))
            ...
            return ("-" if negative else "") + formatted_num
    ...
    def press_number(self, digit):
        ...
        if current_display_str == "0" and digit_upper == "0":
            return

        # For binary mode, only allow 0 and 1
        if self.base_mode == "bin" and digit_upper not in "01":
            return
            
        # For hex mode, allow 0-9 and A-F
        if self.base_mode == "hex" and digit_upper not in "0123456789ABCDEF":
            return
                
        # For decimal mode, allow 0-9
        if self.base_mode == "dec" and digit_upper not in "0123456789":
            return

        if current_display_str == "0" or current_display_str == "-0":
            new_num_str = ("-" if "-" in current_display_str else "") + digit_upper
        else:
            new_num_str = current_display_str + digit_upper

        # Validate length
        num_digits = len(new_num_str.replace("-", ""))
        if self.base_mode == "bin":
            max_digits = self.bit_width
        elif self.base_mode == "hex":
            max_digits = self.bit_width // 4
        else:
            max_digits = 6  # Sufficient for 16-bit numbers

        if num_digits > max_digits:
            return

        # Update the current number and ensure proper display formatting
        self.current_number = self._validate_input_string_to_int(new_num_str)
        # Return the properly formatted display value
...
class CalculatorGUI(QMainWindow):
    ...
    def _create_buttons(self):
        ...
        buttons_data = [
            ...
            ("0", 6, 0, 'digit'), ("+/-", 6, 1, 'sign_toggle'), ("XOR", 6, 2, 'two_op'), ("=", 6, 3, 'equals'),
        ]
        ...
        for btn_text, row, col, btn_type in buttons_data:
            ...
            elif btn_type == 'sign_toggle':
                button.clicked.connect(self._handle_sign_toggle)
    ...
    def _handle_digit_press(self, digit):
        """Handles the pressing of a digit button."""
        new_display_value = self.calculator_logic.press_number(digit)
        if new_display_value is not None:
            self.result_display.setText(new_display_value)

    def _handle_sign_toggle(self):
        current_display = self.operand1_display.text() if not self.calculator_logic.waiting_for_second_operand else self.operand2_display.text()
        
        if current_display and current_display not in ["0", "ERROR", "DIV BY ZERO"]:
            if "-" in current_display:
                new_value = current_display.lstrip("-")
            else:
                if self.calculator_logic.signed_mode:
                    new_value = "-" + current_display
                else:
                    return
            
            if not self.calculator_logic.waiting_for_second_operand:
                self.calculator_logic.current_number = self.calculator_logic._validate_input_string_to_int(new_value)
                self.operand1_display.setText(self.calculator_logic.get_display_value())
            else:
                self.calculator_logic.current_number = self.calculator_logic._validate_input_string_to_int(new_value)
                self.operand2_display.setText(self.calculator_logic.get_display_value())
    ...
    def _set_base_mode(self, new_mode):
        # Get the current base mode before changing
        old_mode = self.calculator_logic.base_mode
        
        # Change to the new base mode first
        self.calculator_logic.set_base_mode(new_mode)
        
        # Helper function to convert between bases
        def convert_value(text, from_base):
            if not text or text in ["ERROR", "DIV BY ZERO"]:
                return text
            ...
            try:
                ...
                # Convert from original base to number
                if from_base == "bin":
                    num = int(text, 2)
                elif from_base == "hex":
                    num = int(text, 16)
                ...
                # Return number formatted in new base
                return self.calculator_logic.get_display_value(num)
            except:
                return text  # Return original text if conversion fails
        
        # Convert and update all displays
        operand1_text = self.operand1_display.text()
        operand2_text = self.operand2_display.text()
        result_text = self.result_display.text()
        
        self.operand1_display.setText(convert_value(operand1_text, old_mode))
        self.operand2_display.setText(convert_value(operand2_text, old_mode))
        self.result_display.setText(convert_value(result_text, old_mode))
