...
class AnalogButton(QPushButton):
	"""
	A custom QSelectorButton that simulates an analog button with a drop shadow effect
	and a visual "press" effect.

	This class extends QSelectorButton to provide a more visually appealing button
...
class ToggleButton(AnalogButton):
	...
	def __init__(self, active_image_file_name, inactive_image_file_name):
		...
		"""
		super().__init__(os.path.join(BUTTON_IMAGE_PATH, active_image_file_name))
		self.active_image_path = os.path.join(BUTTON_IMAGE_PATH, active_image_file_name)
		self.inactive_image_path = os.path.join(BUTTON_IMAGE_PATH, inactive_image_file_name)
...
class SelectorButton(AnalogButton):
	"""
	...
	def __init__(self, image_file_name, toggle_buttons):
		"""
		Initializes the SelectorButton.
		...
		Args:
			...
			toggle_buttons (list[ToggleButton]): A list of ToggleButton instances that this
												button will control.
		"""
		super().__init__(os.path.join(BUTTON_IMAGE_PATH, image_file_name))
...
#
			...
			toggle_button1 = ToggleButton(toggle1_on_image + ".png", toggle1_off_image + ".png")

			toggle2_on_image = list(button_data.keys())[3]
			toggle2_off_image = toggle2_on_image + "_inactive"
			toggle_button2 = ToggleButton(toggle2_on_image + ".png", toggle2_off_image + ".png")

			selector_file_name = list(button_data.keys())[0]
			selector_button = SelectorButton(selector_file_name + ".png", [toggle_button1, toggle_button2])
