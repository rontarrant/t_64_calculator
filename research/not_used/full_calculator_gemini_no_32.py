...
class CalculatorLogic:
	...
	def _validate_input_string_to_int(self, num_str):
		try:
			...
			return 0 # Or raise an error to indicate invalid input
	...
	def get_display_value(self):
		try:
			...
			if self.base_mode == "bin":
				...
				# Hex characters: bit_width / 4. E.g., 8-bit = 2 chars, 16-bit = 4 chars
	...
	def press_number(self, digit):
		...
		# Get the current display value (already formatted according to base mode)
		...
		# Prevent multiple leading zeros for non-binary/hex when value is already '0'
		...
		# For hex/bin modes, we need to strip any leading formatting (like 0x or 0b if present)
		# Also remove leading zeros for proper concatenation
		if self.base_mode == "hex" or self.base_mode == "bin":
			...
			if not stripped_str:  # If we stripped all zeros
				stripped_str = "0"
			current_display_str = stripped_str

		# Construct the new number string
