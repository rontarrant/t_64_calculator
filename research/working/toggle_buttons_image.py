...
class ToggleButton(AnalogButton):
	...
	def __init__(self, button_data):
		...
		"""
		self.active_image = os.path.join(BUTTON_PATH, button_data["name"] + ".png")
		self.inactive_image = os.path.join(BUTTON_PATH, button_data["name"] + "_inactive" + ".png")
		super().__init__(self.active_image)
	...
	def on_enabled_changed(self, enabled):
		...
		"""
		if enabled:
			self.change_image(self.active_image)
		else:
			self.change_image(self.inactive_image)
...
class PushButton(AnalogButton):
	...
	Attributes:
		toggle_buttons (list[ToggleButton]): A list of ToggleButton instances that this
											button controls.
	"""
	def __init__(self, button_data, toggle_buttons):
		"""
...
class MainWindow(QMainWindow):
	def __init__(self):
		...
		toggle_1_data = button_data["button_f"]
		toggle_2_data = button_data["button_e"]
		toggler_data = button_data["button_hex"]

		toggle_button1 = ToggleButton(toggle_1_data)
		toggle_button2 = ToggleButton(toggle_2_data)

		toggler = PushButton(toggler_data, [toggle_button1, toggle_button2])
