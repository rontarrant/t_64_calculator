...
from PySide6.QtCore import QSize, QEvent # Import QSize and QEvent
...
# ... (your button_data dictionary remains the same) ...
button_data = {
# row 1
"bin": {
	"label": "BIN",
...
if __name__ == "__main__":
	...
	class ResizableButton(QPushButton):
		...
		def adjust_font_size(self):
			# Calculate font size based on current button dimensions
		...
		def sizeHint(self):
			# Provide a size hint based on the current font size
			# This helps the layout understand the preferred size
			font_metrics = QFontMetrics(self.font())
			text_size = font_metrics.size(0, self.text())

			# Add some padding to the text size for the button's border and margins
			# You might need to adjust the padding value
			padding = 10
			return QSize(text_size.width() + padding, text_size.height() + padding)

		def minimumSizeHint(self):
			# Provide a minimum size hint, often based on a minimum readable font size
			# Use the enforced minimum font size (6) to calculate the minimum hint
			font = self.font()
			original_point_size = font.pointSize() # Store original size
			font.setPointSize(6) # Temporarily set to min size for calculation
			self.setFont(font) # Apply temporarily
			...
			# Add padding
			padding = 10 # Use the same padding as sizeHint
			return QSize(text_size.width() + padding, text_size.height() + padding)
