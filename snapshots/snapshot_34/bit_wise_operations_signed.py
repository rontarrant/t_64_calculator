...
def bitwise_and(a, b, bit_width):
    ...
    return _normalize(result, bit_width)
...
def shift_left(a, num_bits, bit_width):
    """
    ...
    return _normalize(shifted_val, bit_width)

def shift_right_logical(a, num_bits, bit_width):
    """
    Performs a logical right shift on a signed integer.
...
def shift_right_arithmetic(a, num_bits, bit_width):
    """
    ...
    shifted_val = actual_signed_a >> num_bits
...
if __name__ == "__main__":
    ...
    print(f"AND({a_8}, {b_8}): {bitwise_and(a_8, b_8, 8)}") # Expected: 0 (0000 0000)
    print(f"OR({a_8}, {b_8}): {bitwise_or(a_8, b_8, 8)}")   # Expected: -5 (1111 1011)
    print(f"XOR({a_8}, {b_8}): {bitwise_xor(a_8, b_8, 8)}") # Expected: -5 (1111 1011)
    print(f"NOT({a_8}): {bitwise_not(a_8, 8)}")             # Expected: -6 (1111 1010)
    print(f"NOT({b_8}): {bitwise_not(b_8, 8)}")             # Expected: 9 (0000 1001)

    print(f"Shift Left ({a_8}, 1): {shift_left(a_8, 1, 8)}")        # Expected: 10
    print(f"Shift Left ({c_8}, 1): {shift_left(c_8, 1, 8)}")        # Expected: -2 (0111 1111 -> 1111 1110) - overflowed
    print(f"Shift Left ({d_8}, 1): {shift_left(d_8, 1, 8)}")        # Expected: 0 (1000 0000 -> 0000 0000) - overflowed

    print(f"Shift Right Logical ({b_8}, 1): {shift_right_logical(b_8, 1, 8)}") # Expected: 123 (1111 0110 -> 0111 1011) - zeros shifted in
    print(f"Shift Right Arithmetic ({b_8}, 1): {shift_right_arithmetic(b_8, 1, 8)}") # Expected: -5 (1111 0110 -> 1111 1011) - sign bit extended
    print(f"Shift Right Logical ({c_8}, 1): {shift_right_logical(c_8, 1, 8)}") # Expected: 63 (0111 1111 -> 0011 1111)
    print(f"Shift Right Arithmetic ({c_8}, 1): {shift_right_arithmetic(c_8, 1, 8)}") # Expected: 63 (0111 1111 -> 0011 1111)
    ...
    print(f"AND({a_16}, {b_16}): {bitwise_and(a_16, b_16, 16)}") # Expected: 256
    print(f"OR({a_16}, {b_16}): {bitwise_or(a_16, b_16, 16)}")   # Expected: -1
    print(f"XOR({a_16}, {b_16}): {bitwise_xor(a_16, b_16, 16)}") # Expected: -257
    print(f"NOT({a_16}): {bitwise_not(a_16, 16)}")               # Expected: -257
    print(f"NOT({b_16}): {bitwise_not(b_16, 16)}")               # Expected: 0

    print(f"Shift Left ({a_16}, 1): {shift_left(a_16, 1, 16)}")        # Expected: 512
    print(f"Shift Left ({c_16}, 1): {shift_left(c_16, 1, 16)}")        # Expected: -2 (0111... -> 1111...1110) - overflow
    print(f"Shift Left ({d_16}, 1): {shift_left(d_16, 1, 16)}")        # Expected: 0 (1000... -> 0000...0000) - overflow

    print(f"Shift Right Logical ({b_16}, 1): {shift_right_logical(b_16, 1, 16)}") # Expected: 32767
    print(f"Shift Right Arithmetic ({b_16}, 1): {shift_right_arithmetic(b_16, 1, 16)}") # Expected: -1
    print(f"Shift Right Logical ({d_16}, 1): {shift_right_logical(d_16, 1, 16)}") # Expected: 16384
    print(f"Shift Right Arithmetic ({d_16}, 1): {shift_right_arithmetic(d_16, 1, 16)}") # Expected: -16384
