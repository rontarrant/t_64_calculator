...
if __name__ == '__main__':
	...
	window = MainWindow()
	window.show()
	# In main.py, after the window is shown (window.show())

	# --- Test AnalogButton active/inactive state ---
	if "bin" in window.buttons:
		bin_button = window.buttons["bin"]
		print(f"Initial active state of BIN button: {bin_button._is_active}")
		bin_button.set_active(False)
		bin_button.update()
		print(f"Active state of BIN button after setting to False: {bin_button._is_active}")

	if "bit_16" in window.buttons:
		bit_16_button = window.buttons["bit_16"]
		print(f"Initial active state of 16-BIT button: {bit_16_button._is_active}")
		bit_16_button.set_active(False)
		bit_16_button.update()
		print(f"Active state of 16-BIT button after setting to False: {bit_16_button._is_active}")

	# You should now see the "BIN" and "16-BIT" buttons drawn with the default
	# inactive appearance (which is currently the same as active since we haven't
	# implemented the palette usage in the paintEvent yet). The print statements
	# in the console will also confirm the _is_active flag has changed.
	# --- End of Test ---
	app.exec()
