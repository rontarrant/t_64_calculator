...
def set_math_operation(main_window):
	...
	global signed_state
	sign_button = main_window.sign_button
	...
	if first_input_label.text() != "":
		...
		if sign_button._text == "-":
			switch_sign(main_window)
			sign_button._text = "+"
			sign_button.update()
...
def switch_sign(main_window):
	global sign_button_state
	global current_label
	global signed_state
	...
	clicked_button = main_window.sender()
	#print("button: ", clicked_button.text())

	# signed_state is set (below) by the toggle-sign button
	if signed_state == True:
	...
	clicked_button.set_active(signed_state)
...
def toggle_signed(main_window):
	...
	# if we're in signed mode, go to unsigned mode
		...
		if sign_button._text == sign_button.properties["alt_label"]:
			# change the button text to "+"
			sign_button._text = sign_button.properties["label"]
	...
	sign_button.set_active(signed_state)
	sign_button.setEnabled(signed_state)
	sign_button.update()
