...
class MainWindow(QMainWindow):
	def __init__(self):
		...
		self.scaling_factor = dpi / 96.0  # Assuming 96 DPI as the base
			...
			for column in range(columns):
				...
				if index < len(button_ids):
					...
					if button_type == "radio" and properties["group"] == "numsys":
						...
						button = OneLineButton(specs)
						button.clicked.connect(self.handle_digit_click_wrapper)
					...
					if button_type != "radio" and properties["group"] != "digit":
	...
	def handle_digit_click_wrapper(self):
		from callbacks import handle_digit_click
		handle_digit_click(self)
