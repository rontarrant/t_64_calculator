...
class MainWindow(QMainWindow):
	def __init__(self):
		...
		#self.first_input.setFocusPolicy(Qt.StrongFocus)
		self.first_input.setTextInteractionFlags(Qt.TextEditable) # Allow text selection/editing
		...
		register_labels(self.first_input, self.second_input, self.operation_label,
							 self.results_label, self.remainder_label)

		# Initialize button dictionaries
		self.buttons = {}
		self.numsys_buttons = {}
		self.bitwidth_buttons = {}
		self.sign_set_button = None
	...
	def _create_input_area(self):
		...
		self.button_layout.addWidget(self.remainder_label, 2, 3)
	...
	def _create_button_grid(self, start_row = 0):
			...
			for column in range(columns):
					...
					if index < len(button_ids):
						...
						if button_type == "radio" and properties["group"] == "numsys":
							...
							elif properties["group"] == "sign_mode":
									button.clicked.connect(self.signed_button_proxy)
							elif properties["group"] == "about":
								button.clicked.connect(self.about_proxy)
							elif properties["group"] == "sign_set":
								button.clicked.connect(self.set_sign_proxy)
								self.sign_set_button = button
						else:
		...
		equals_button.clicked.connect(self.equals_proxy)
		self.button_layout.addWidget(equals_button, start_row + 9, 0, 1, 4)
	...
	def set_sign_proxy(self):
		from callbacks import set_sign
		set_sign(self)
	...
	def signed_button_proxy(self):
		from callbacks import sign_mode_toggle
		sign_mode_toggle(self)
