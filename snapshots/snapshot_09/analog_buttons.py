...
class AnalogButton(QPushButton):
	def __init__(self, properties, parent = None):
		...
		self._scaling_factor = properties.get("scaling_factor", 1.0)
		self._top_width = properties.get("width", 100) - (5 * self._scaling_factor)
		self._top_height = properties.get("height", 50) - (5 * self._scaling_factor)
...
class OneLineButton(AnalogButton):
	...
	def paintEvent(self, event):
		...
		radius = 15 * (self.window().scaling_factor if self.window() else 1)
		button_area_rect = self.rect()
		...
		font = QFont(self._font_family, self._font_size * (self.window().scaling_factor if self.window() else 1))
		painter.setFont(font)
		metrics = painter.fontMetrics()
		text_rect = metrics.boundingRect(self._text)
		text_x = button_area_rect.center().x() - text_rect.width() / 2
		baseline_y = button_area_rect.center().y() + metrics.ascent() / 2 - (10 * (self.window().scaling_factor if self.window() else 1)) # Scale the offset
		...
		# Draw the outline
		outline_width = self.properties.get("specs", {}).get("outline_width", 4) * (self.window().scaling_factor if self.window() else 1)
		painter.setPen(QPen(outline_color, outline_width))
		painter.setBrush(Qt.NoBrush)
		area = outline_width // 2
		corner_r = radius - (2 * (self.window().scaling_factor if self.window() else 1))
		adjusted_outline = top_rect.adjusted(area, area, -area, -area)
		painter.drawRoundedRect(adjusted_outline, corner_r, corner_r)

		# Draw the text using the dedicated method (adjust font size here too)
		self._draw_centered_text(painter, top_rect, self._text, self._font_family, self._font_size * (self.window().scaling_factor if self.window() else 1), text_color)
	...
	def _draw_centered_text(self, painter, rect, text, font_family, font_size, color):
		...
		baseline_y = rect.center().y() + metrics.ascent() / 2 - (10 * (self.window().scaling_factor if self.window() else 1)) # Scale the offset
...
class TwoLineButton(AnalogButton):
	...
	def paintEvent(self, event):
		...
		radius = 15 * self._scaling_factor
		button_area_rect = self.rect()
		top_width = self._top_width
		top_height = self._top_height
		shadow_rect = QRect(button_area_rect.bottomRight() - QPoint(top_width, top_height), QSize(top_width, top_height))
		...
		# Draw the outline
		outline_width = self.properties.get("specs", {}).get("outline_width", 4) * self._scaling_factor
		painter.setPen(QPen(outline_color, outline_width))
		painter.setBrush(Qt.NoBrush)
		area = outline_width // 2
		corner_r = radius - (2 * self._scaling_factor)
		adjusted_outline = top_rect.adjusted(area, area, -area, -area)
		painter.drawRoundedRect(adjusted_outline, corner_r, corner_r)

		# Draw the main text (relative to top, with scaling for vertical position)
		main_font = QFont(self._font_family, self._font_size * self._scaling_factor)
		...
		# Draw the subtext (relative to top, with scaling for vertical position)
		sub_font = QFont(self._font_family, self._subfont_size * self._scaling_factor)
...
class AngledButton(AnalogButton):
	...
	def paintEvent(self, event):
		...
		font = QFont(self._font_family, self._font_size * self._scaling_factor)
		painter.setFont(font)
		metrics = painter.fontMetrics()
		text_rect = metrics.boundingRect(self._text)
		text_x = top_rect.center().x() - text_rect.width() / 2
		vertical_offset = 5 * self._scaling_factor  # This is the line you should have
