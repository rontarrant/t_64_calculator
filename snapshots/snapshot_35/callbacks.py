...
def do_equals(main_window): # equals button
	...
	if operation_flag != None:
		...
		if signed_mode == False:
			...
			match operation_flag:
				case "+":
					result = add_signed_bits(first_number, second_number, bitwidth)
				case "-":
					result = subtract_signed_bits(first_number, second_number, bitwidth)
				case "*":
					result = multiply_signed_bits(first_number, second_number, bitwidth)
				case "/":
					result = divide_signed_bits(first_number, second_number, bitwidth)
		...
		if operation_flag == "/" and result[2] == True:
			display_result = "NaN"
